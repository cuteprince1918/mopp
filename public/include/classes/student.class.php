<?php
    class student_master extends db{
        public $errors = '';
            ...
            if($this->errors!=''){
				...
				if($this->errors!=''){
                    ...
                    if($id==0){
						...
						$q = "INSERT INTO `".$this->table."` SET `name`='".$name."',`email`='".$email."', `image`='".$file_image."', `username`='".$username."', `password`='".$this->encryptor($password)."' ".$this->insert_common_sql();
						$res = $this->re_db_query($q);
						...
						$q = "UPDATE `".$this->table."` SET `name`='".$name."',`email`='".$email."',`username`='".$username."' ".$con." ".$this->update_common_sql()." WHERE `id`='".$id."'";
						$res = $this->re_db_query($q);
						if($res){
